...
yum clean all
yum repolist
yum install -y puppet-agent rsync lvm2 gcc-c++ make cmake net-tools sysstat dstat git epel-release nedit ntp ntpdate telnet mlocate lsof bind-utils
...
cp /vagrant/files/puppet/puppet-run.* /usr/lib/systemd/system
